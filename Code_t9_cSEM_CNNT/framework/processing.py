...
def training_process_cSEM_old(generator, model, cuda, models_dir,
                                      training_label_type, epochs=config.epochs,
                                      batch_size=config.batch_size,  lr_init = config.lr_init,):
    create_folder(models_dir)
    ...
    for iteration, all_data in enumerate(generator.generate_train()):
        ...
        batch_x = move_data_to_gpu(batch_x, cuda)
        batch_y = move_data_to_gpu(batch_y, cuda)
        # batch_y_event = move_data_to_gpu(batch_y_event_cpu, cuda, half=True)
        batch_y_event = move_data_to_gpu(batch_y_event, cuda)
        ...
        x_scene_linear, x_event_linear, scene_w, event_w, x_scene_embed, x_event_embed = model(batch_x)
        # print(x_scene_linear.size(), x_event_linear.size(),
        #       scene_w.size(), event_w.size(),
        #       x_scene_embed.size(), x_event_embed.size())
        # # torch.Size([1, 10]) torch.Size([1, 527])
        # # torch.Size([10, 768]) torch.Size([527, 768])
        # # torch.Size([1, 768]) torch.Size([1, 768])

        x_event_sigmoid = F.sigmoid(x_event_linear)
        # print(x_event_sigmoid.dtype, batch_y_event.dtype)   # torch.float16, torch.float32
        # half: 实际结果 torch.float16 torch.float16
        loss_event = event_loss(x_event_sigmoid, batch_y_event)

        x_scene_softmax = F.log_softmax(x_scene_linear, dim=-1)
        # print(x_scene_softmax.dtype, batch_y.dtype)  # torch.float16 torch.int64
        loss_scene = F.nll_loss(x_scene_softmax, batch_y)
