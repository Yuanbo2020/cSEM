...
class MlhE_plus_PANN(nn.Module):
    def __init__(self, classes_num, event_class, batchnormal=False):

        super(MlhE_plus_PANN, self).__init__()
        ...
        self.conv_block6 = ConvBlock(in_channels=1024, out_channels=2048)
        self.fc1 = nn.Linear(2048, 2048, bias=True)
        self.fc1_another_layer = nn.Linear(2048, 2048, bias=True)
        self.fc_final = nn.Linear(2048, classes_num, bias=True)
        ...
        self.fc1_event = nn.Linear(2048, 2048, bias=True)
        self.fc1_event_another_layer = nn.Linear(2048, 2048, bias=True)
        self.fc_audioset = nn.Linear(2048, event_class, bias=True)
    ...
    def init_weight(self):
        ...
        init_layer(self.fc1_another_layer)
        init_layer(self.fc1_event_another_layer)
    ...
    def forward(self, input):
        ...
        x_scene_embed = F.relu_(self.fc1_another_layer(x_scene_embed))
        ...
        x_event_embed = F.relu_(self.fc1_event_another_layer(x_event_embed))

        event_linear = self.fc_audioset(x_event_embed)
        # print(event.size())  torch.Size([64, 527])

        return scene, event_linear
