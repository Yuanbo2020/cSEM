...
class PANN(nn.Module):
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        output_dict = {'clipwise_output': clipwise_output, 'embedding': embedding, 'stft': stft, 'logmel': logmel}
...
class AST(nn.Module):
    def __init__(self, scene_dim=10, total_layers=10):
        ...
        num_heads = 12
        # 如果将12改为其他的，会影响attention中剩余通道的参数
        # qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, total_layers)]  # stochastic depth decay rule
        ...
        self.shared_fc_2048 = nn.Linear(embed_dim, my_dim, bias=True)

        self.fc_final_asc = nn.Linear(my_dim, scene_dim, bias=True)

        self.spec_augmenter = SpecAugmentation(time_drop_width=64, time_stripes_num=2,
                                               freq_drop_width=16, freq_stripes_num=4)
    ...
    def my_init_weight(self):
        init_layer(self.shared_fc_2048)
    ...
    def forward(self, x):
        ...
        if self.training:
            x = self.spec_augmenter(x)
            # print('spec_augmenter x.size():', x.size())
            # spec_augmenter x.size(): torch.Size([2, 1, 1024, 128])
        ###################################################################
        ...
        # print(x.size())   torch.Size([2, 1214, 768])
        x = self.norm_scene(x)
        # print(x.size())
        x = (x[:, 0] + x[:, 1]) / 2

        x_embed = F.relu_(self.shared_fc_2048(x))
        # print(x_embed.size())  torch.Size([2, 2048])

        x_sce = self.fc_final_asc(x_embed)
        # print(x_sce.size())  # torch.Size([2, 10])
...
class cSEM_AST(nn.Module):
    ...
    def forward(self, x):
        ...
        x_sce = (x_sce[:, 0] + x_sce[:, 1]) / 2   # 取分类头和蒸馏头的结果做平均
        ...
        x_eve = (x_eve[:, 0] + x_eve[:, 1]) / 2   # 取分类头和蒸馏头的结果做平均
...
class MoE_AST(nn.Module):
    def __init__(self, scene_dim=10, event_dim=527, depth=10):

        super(MoE_AST, self).__init__()
    ...
    def forward(self, x):
        ...
        x_eve = self.fc_final_event(x_embed)
        # print(x_eve.size())  # torch.Size([2, 527])
...
class MlhE_AST(nn.Module):
    def __init__(self, scene_dim=10, event_dim=527, shared_layers=4, depth=10):

        super(MlhE_AST, self).__init__()
    ...
    def forward(self, x):
        ...
        return x_sce, x_eve, scene_w, event_w, x_scene_embed, x_event_embed
