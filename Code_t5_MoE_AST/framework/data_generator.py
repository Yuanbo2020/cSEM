...
def ast_normalize_2019():
    dir_path = os.path.join(config.dataset_dir, 'AST')
    hdf5_file = os.path.join(dir_path, '1_ast_D2019T1A_training.h5')

    if not os.path.exists(hdf5_file):
        meta_csv = os.path.join(config.subdir, 'meta.csv')
        ...
        for row in df.iterrows():
            ...
            audio_names.append(audio_name.split('.wav')[0])
            scene_labels.append(scene_label)

        input_tdim = 1024
        mel_bins = 128

        file_path = os.path.join(dir_path, '0_ast_D2019T1A_feature_original_998_128.pickle')
        feature_set = pickle.load(open(file_path, 'rb'))

        # Create hdf5 file
        hf = h5py.File(hdf5_file, 'w')
        hf.create_dataset(name='feature', shape=(0, input_tdim, mel_bins),
                          maxshape=(None, input_tdim, mel_bins), dtype=np.float32)
        ...
        for (n, key) in enumerate(audio_names):
            fbank = feature_set[key]
            n_frames = fbank.shape[0]
            p = input_tdim - n_frames
            if p > 0:
                m = torch.nn.ZeroPad2d((0, 0, 0, p))
                fbank = m(fbank)
            elif p < 0:
                fbank = fbank[0:input_tdim, :]

            fbank = (fbank - (-4.2677393)) / (4.5689974 * 2)

            hf['feature'].resize((n + 1, input_tdim, mel_bins))
            hf['feature'][n] = fbank
        hf.close()
    else:
        print('Done: ', hdf5_file)


class DataGenerator_ast_scene_event_add_onthot_scene(object):
    def __init__(self,  hdf5_path, event_label_type, label_type, batch_size=config.batch_size,
                 dev_train_csv=config.dev_train_csv,
                 dev_validate_csv=config.dev_validate_csv, seed=42, normalization=False):
        """
        Inputs:
          hdf5_path: str
          batch_size: int
        ...
        # print(len(self.audio_names))

        self.pseudo_tagging_labels = []
        event_label_dir = os.path.join(config.ast_dir, '1_integrate_event_labels', event_label_type)

        self.pseudo_label_file = os.path.join(config.ast_dir,
                                              'ast_D2018T1A_event' + label_type + '_mAP_0485.npy')

        if not os.path.exists(self.pseudo_label_file):
            for each in self.audio_names:
                filename = each + '_matrix.txt'
                filepath = os.path.join(event_label_dir, filename)
                if os.path.exists(filepath):
                    data = np.loadtxt(filepath)
                    if 'hard' in label_type:
                        threshold_event_presence_absence = 0.1
                        hard_data = np.zeros_like(data)
                        hard_data[data > threshold_event_presence_absence] = 1  # (527, )
                        self.pseudo_tagging_labels.append(hard_data)
                    elif 'soft' in label_type:
                        self.pseudo_tagging_labels.append(data)
                else:
                    print('File does not exist: ', filepath)
                    break
            self.pseudo_tagging_labels = np.array(self.pseudo_tagging_labels)
            np.save(self.pseudo_label_file, self.pseudo_tagging_labels)
        else:
            print('Pseudo label file : ', self.pseudo_label_file)
            self.pseudo_tagging_labels = np.load(self.pseudo_label_file)

        self.x = hf['feature'][:]
        # print(self.x.shape)  # (8640, 1024, 128)
        # self.x = self.x[:, :998]
        # print(self.x.shape)  # (8640, 998, 128)

        self.scene_labels = [s.decode() for s in hf['scene_label'][:]]
        self.y = np.array([lb_to_ix[lb] for lb in self.scene_labels])
        # print(self.y.shape)
        self.y_onehot = np.zeros((len(self.scene_labels), len(lb_to_ix)))
        # print(self.y.shape)
        for num, lb in enumerate(self.scene_labels):
            # print(num, lb_to_ix[lb])
            self.y_onehot[num, lb_to_ix[lb]] = 1
        hf.close()
        ...
        # Split data to training and validation
            ...
            self.test_audio_indexes = self.get_audio_indexes_from_csv(
                config.dev_evaluate_csv)
            ...
            print('Test data {}.'.format(len(self.test_audio_indexes)))
    ...
    def get_audio_indexes_from_csv(self, csv_file):
        ...
        for li in lis:
            audio_name = li[0].split('/')[1].split('.wav')[0]
    ...
    def generate_train(self):
        ...
        while True:
            ...
            batch_y_onehot = self.y_onehot[batch_audio_indexes]

            yield batch_x, batch_y, batch_y_event, batch_y_onehot
    ...
    def generate_validate(self, data_type, shuffle, devices=['a'], max_iteration=None):
        ...
        for n in range(len(audio_indexes)):
            if self.audio_names[audio_indexes[n]].split('-')[-1] in devices:
        ...
        while True:
            ...
            # Get batch indexes
            batch_audio_indexes = devices_specific_indexes[pointer: pointer + batch_size]
